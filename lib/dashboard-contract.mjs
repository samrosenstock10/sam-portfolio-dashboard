import { validateCompanyConcentration } from './company-concentration.mjs';
import { validateEtfFreshness } from './etf-freshness.mjs';
import { validateBusinessExposure } from './business-exposure.mjs';
...
function validateDashboardData(html) {
  ...
  validateEtfFreshness(data);
  validateBusinessExposure(data.businessExposure, data.holdings, data.portfolioAsOf, data);
...
export function validateDashboardHtml(html) {
  ...
  invariant(!/<h2>Economic (?:exposure mix|position-size structure)<\/h2>/i.test(html), 'removed_allocation_section_restored');
  for (const id of ['sector-analysis', 'beta-analysis']) {
    const disclosure = html.match(new RegExp('<details\\b[^>]*id="' + id + '"[^>]*>'));
    invariant(disclosure && !/\bopen(?:\s|=|>)/i.test(disclosure[0]), `section_must_start_collapsed:${id}`);
  }
