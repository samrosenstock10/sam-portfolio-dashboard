...
// These are calendar dates, not local-midnight instants. Date.parse on a
// human-readable date shifts the day when the worker runs east of UTC.
function calendarIso(value) {
  ...
  if (!/^\d{4}-\d{2}-\d{2}$/.test(iso)) {
    ...
    iso = `${match[3]}-${String(months.indexOf(match[1]) + 1).padStart(2, '0')}-${match[2].padStart(2, '0')}`;
  }
...
export function validateEtfFreshness(data) {
  ...
  if (!statusDate || calendarIso(statusDate[1]) !== data.portfolioAsOf) fail('portfolio_status_date_mismatch');
  ...
  for (const fund of ['qqq', 'spy']) {
    ...
    rows.forEach((row, index) => {
      if (calendarIso(row.asOf) !== expected) fail(`mixed_or_missing_table_date:${fund}:${index}`);
