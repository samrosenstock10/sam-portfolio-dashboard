...
Combine GOOG and GOOGL before ranking exactly 30 unique companies. A top-25 paywalled preview is not a top-30 source. Recalculate both ETF contributions, company totals and ranks, residual sleeves, company concentration/effective-position statistics, sector totals, and the affected comparison from the recorded versions. Verify tax-location, vehicle mix, Roth allocation, and direct-stock market-cap totals remain reconciled; ETF-only corrections must not change unrelated holdings or returns.
...
Maintain the existing source spreadsheet 1XrpgOS9dFkQljaUf9Eftk6DyGnyYcmHnoKZGwGoS1hw tabs Business Exposure (1900000101), Business Mapping (1900000102), Business Categories (1900000103), and ETF Remaining Holdings (1900000104). This is part of the Friday portfolio layer and its public artifact. Preserve the repository contract's existing data, source-freshness, financial-method, publication and notification requirements. Maintain the full-width Business exposure card directly after Underlying sector exposure. Use descending horizontal bars: blue direct/other, purple ETF top 30, and gray ETF outside top 30 (including any unclassified fund remainder). Initially show ten categories with a Show all / Show top 10 button. Each native expandable category shows one row per company with portfolio allocation and company business share. Keep long labels legible, controls usable by touch and keyboard, and mobile tables within the viewport. Preserve source dates, the short revenue-basis note and the unclassified ETF weight. Do not add a duplicate bottom table.

Keep internal joins between colored business-bar segments square and flush. Round only the two outer ends of the complete filled bar, including bars with a single nonzero segment.

Include this feature in the cheap incomplete-layer check before declaring COMPLETE: read Business Exposure!B6:B7, Business Categories!A51:B60, ETF Remaining Holdings!A3:H7 and Notes!B2. Include the remaining-holdings verified Friday cursor B59 and readiness B60; pending activation is incomplete even when the base mappings were previously verified. Compare Business Categories!B53 (verified Friday cycle, ISO date) with the latest required Friday and B52 with the actual Notes!B2 holdings snapshot. A complete daily layer or existing weekly-history row does not complete an outstanding business-exposure refresh. If the base Friday source layer is pending, retain truthful feature dates/status and follow the existing recovery rules. Repair missing business maintenance from a verified Friday source snapshot without duplicating Weekly History; publish only if the completed public candidate differs.

When due, read Business Categories!A1:E60 and the current Business Mapping / Business Exposure content and structure before editing. Follow the documented methods and stable common Category IDs. Read the current Holdings, Economic Exposure and ETF Look-Through source tables. Reconcile every economic source position, including new or exited positions, current portfolio weights, cash/gold and the separate QQQ/SPY residuals. Normalize share-class aliases to the exact canonical source ticker. Keep a single active reporting period per company and no duplicate ticker/business/period mapping keys. Reuse a common category for equivalent activities even when companies report different segment names; retain the original company labels alongside the shared category.
...
Use Economic Exposure total weights once, with segment contribution = company portfolio weight × business share. Preserve the direct, QQQ and SPY decomposition. The base mappings retain QQQ ex Top 30 / SPY ex Top 30 in ETF_OTHER. Reallocate those residuals into verified named outside-top-30 constituents using docs/automation/etf-remaining-holdings.md, leaving only the unclassified balance there. Never add tail exposure on top of the residual or infer an aggregate custom mix from standard sector percentages. Reuse full issuer files for both sides of rank 30 and maintain classifications while refreshing weights/membership weekly.
...
Export only the public percentage fields described in docs/automation/business-exposure.md. Use scripts/update-business-exposure.mjs to combine equivalent segments and update dashboard-data.businessExposure. The normal build validates base and outside holdings against the unchanged canonical fund ownership, top-30 membership, residual attribution and dates. Follow docs/automation/etf-remaining-holdings.md for the source tab, initial pending activation and public schema. Preserve the business data and renderer on daily performance-only updates. Do not embed corporate revenue audit columns, gold quantity labels, account values, or the private refresh ledger in the site.

Only after the feature checks pass, set Business Categories B51 to the truthful refresh state, B52 to the verified Notes!B2 snapshot text, B53 to the verified Friday cycle in YYYY-MM-DD, and B54 to the actual business review date. Preserve B55 version and the live coverage/count/freshness formulas in B56:B58. Separately advance B59 only when the remaining-holdings source and publication checks pass; B60 remains a live readiness formula. Business Exposure!E3 references B54. On source failure preserve the last supported mappings, label staleness/pending review honestly and leave the verified-cycle cursor unchanged. Continue recovery and use only the existing final-failure notification rules.
...
The owner approved this streamlined layout on September 11, 2026. Do not restore Economic exposure mix or Economic position-size structure. Underlying sector exposure (`sector-analysis`) and detailed 1-year beta (`beta-analysis`) are native details disclosures, closed on initial load. Keep the headline beta metric and all source tables. Keep Tax location full-width and leave market-cap allocation unchanged.

After building Business Exposure, assign the result of `companyConcentration(data.businessExposure)` from `lib/company-concentration.mjs` into `data.stats`. The normal business import does this automatically. Recombine all category allocations by canonical company ticker, including only verified active ETF constituents. Exclude CASH, XAU and both ETF remainder tickers. Never treat an ETF remainder as one company or add its exposure on top of its constituents. Largest/top-3/top-5/top-10 values are shares of the total portfolio for identified companies. `effectivePositions` is normalized **within identified companies** (`knownCompanyShare` squared divided by their sum of squared portfolio weights). Its public label must say Known-company equivalent and show coverage; it is not a complete-portfolio diversification score. Preserve these definitions on daily updates; the validator independently recalculates all six metrics.

Dashboard!A8:B9 use Largest known company / Top 5 known companies and live grouped formulas over Business Mapping A/K plus ETF Remaining Holdings C/J, excluding CASH, XAU and both fund remainders. Keep these formulas through weekly updates; extend their ranges if either source grows. Include all reported segments exactly once, not category subtotals. Source rows whose tail contributions are zero remain inactive for calculation purposes. Historical Weekly History rows retain their original metric definitions; do not rewrite old snapshots as company concentration. For public week-over-week comparisons, recalculate both dates on the same identified-company basis from verified source positions, or omit the comparison. Never compare a new company-only metric with an old metric that counted pooled ETF remainders. Keep the changes narrative and card metadata consistent with the current metrics. The holdings-chart footnote may say remainders remain in total portfolio allocation; it must not claim they are included in every calculation.
